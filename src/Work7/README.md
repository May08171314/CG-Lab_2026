...
# 6. 拓展选做功能实现
## （1） 完整三类弹簧布料模型
在基础结构弹簧之上，新增**剪切弹簧（对角线）**与**弯曲弹簧（隔点约束）**：
- 剪切弹簧：约束布料斜向拉伸，抑制网格菱形畸变；
- 弯曲弹簧：限制布料过度弯折，让布料整体更平整挺括；
对照现象：仅结构弹簧布料易拉伸变形，增加剪切+弯曲弹簧后布料褶皱更自然，不易扭曲。

## （2） 球体空间碰撞模拟
场景内置固定球体，物理受力计算时遍历所有质点，计算质点与球心距离：
1. 若质点距离小于球体半径，判定发生穿透；
2. 根据穿透深度与球体法向生成碰撞弹力，将质点推出球体；
3. 3D渲染白色碰撞球体，布料下落撞击球体后会产生回弹，不会穿入球体内部。

| extra1 | extra2 |
|--------|--------|
| ![extra1](gifs/extra1.gif) | ![extra2](gifs/extra2.gif) |
