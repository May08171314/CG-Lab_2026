...
# 1. 项目架构
本项目严格遵循实验文档GPU并行同步规范，采用**分层模块化架构**，整体分为5大模块，模块间解耦、满足Kernel拆分、ti.func内联性能优化要求：
```
├── 全局参数与Taichi数据场定义
├── 初始化模块（多Kernel拆分，保证GPU同步）
│   ├── init_positions：质点位置、速度、固定约束初始化
│   ├── init_springs：生成结构弹簧拓扑、记录原长
│   └── init_spring_indices：预处理弹簧渲染索引
├── 底层力学工具函数（ti.func内联）
│   ├── compute_forces_on：统一计算重力、阻尼、弹簧弹力（原子加锁防线程冲突）
│   └── clamp_velocity：速度钳制，解决数值爆炸问题
├── 三大积分求解Kernel（核心物理求解器）
│   ├── step_explicit：显式欧拉积分
│   ├── step_semi_implicit：半隐式（辛）欧拉积分
│   └── step_implicit_iter：定点迭代隐式欧拉积分
└── 主渲染与GGUI交互模块
    ├── 3D场景、相机、光照渲染管线
    ├── GGUI交互控制面板（切换积分器、暂停、重置）
```
架构约束说明：
...
1. `dt = 1e-3`（大步长）：轻微抖动，极限下会不稳定；
2. `dt = 1e-4`（小步长）：模拟极度平滑，计算耗时小幅上升。
