...
# ===================== 选做 2：硬阴影函数 =====================
@ti.func
def is_in_shadow(shadow_ro, light_pos):
    """
    发射阴影射线，判断点是否在阴影中
    shadow_ro: 物体表面交点 (稍微偏移防止自身遮挡)
    light_pos: 光源位置
    返回 True = 在阴影中，False = 被照亮
    """
    shadow_rd = normalize(light_pos - shadow_ro)
    light_dist = (light_pos - shadow_ro).norm()
    
    # 阴影射线与球体求交
    t_sph, _ = intersect_sphere(shadow_ro, shadow_rd, ti.Vector([-1.2, -0.2, 0.0]), 1.2)
    # 阴影射线与圆锥求交
    t_cone, _ = intersect_cone(shadow_ro, shadow_rd, ti.Vector([1.2, 1.2, 0.0]), -1.4, 1.2)
    
    in_shadow = False
    # 交点在物体与光源之间 → 在阴影里
    if 0 < t_sph < light_dist - 1e-3:
        in_shadow = True
    if 0 < t_cone < light_dist - 1e-3:
        in_shadow = True
    return in_shadow
...
def render():
    for i, j in pixels:
        ...
        if min_t < 1e9:
            ...
            # ===================== 选做 2：硬阴影判断 =====================
            shadow_bias = N * 1e-3  # 微小偏移，防止自身遮挡
            p_shadow = p + shadow_bias
            in_shadow = is_in_shadow(p_shadow, light_pos)
...
def main():
    window = ti.ui.Window("Phong + Blinn-Phong + Shadow", (res_x, res_y))
