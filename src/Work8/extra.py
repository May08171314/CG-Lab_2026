...
# ====================== 新增：选做动画生成函数 ======================
def generate_joint_animation(model, betas, joint_name, axis, start_rad=0.0, end_rad=0.8, frame_num=30, out_gif_path="outputs/anim_arm_rotate.gif"):
    """
    选做任务：生成单关节平滑旋转GIF动画
    :param model: 加载完成的SMPL模型
    :param betas: 固定不变的体型参数
    :param joint_name: 目标关节名称
    :param axis: 旋转轴 [x,y,z]
    :param start_rad: 起始旋转弧度
    :param end_rad: 终止旋转弧度
    :param frame_num: 总插值帧数
    :param out_gif_path: 输出GIF路径
    """
    device = betas.device
    dtype = betas.dtype
    joint_mapping = {
    ...
    pose_start_offset = (joint_idx - 1) * 3

    temp_frame_list = []
    # 线性插值生成每一帧旋转角度
    angle_sequence = np.linspace(start_rad, end_rad, frame_num)

    for frame_id, rot_angle in enumerate(angle_sequence):
        # 全局朝向固定为0
        global_orient = torch.zeros((1, 3), dtype=dtype, device=device)
        # 全部关节姿态清零，只修改目标关节
        body_pose = torch.zeros((1, 23 * 3), dtype=dtype, device=device)
        rot_vector = torch.tensor(axis, dtype=dtype, device=device) * rot_angle
        body_pose[0, pose_start_offset : pose_start_offset + 3] = rot_vector

        # 执行手写完整LBS流程
        stage_data = compute_manual_lbs(model, betas, global_orient, body_pose)
        current_verts = to_numpy(stage_data["verts"][0])
        current_joints = to_numpy(stage_data["J_transformed"][0])

        # 临时保存单帧图片
        temp_png = os.path.join(os.path.dirname(out_gif_path), f"_anim_temp_{frame_id:03d}.png")
        save_single_figure(
            temp_png,
            current_verts,
            to_numpy(model.faces),
            joints=current_joints,
            title=f"Frame {frame_id} | {joint_name} Rot = {rot_angle:.2f} rad"
        )
        temp_frame_list.append(temp_png)

    # 读取全部帧合成GIF
    gif_frames = [imageio.imread(img_path) for img_path in temp_frame_list]
    imageio.mimsave(out_gif_path, gif_frames, fps=10)
    print(f"\n✅ 选做动画GIF已生成：{out_gif_path}")

    # 清理临时png帧文件
    for tmp_png in temp_frame_list:
...
def main(args):
    ...
    print(f"静态结果图片已保存到: {out_dir}")

    # ====================== 执行选做动画任务 ======================
    print("\n===== 开始执行选做：单关节旋转姿态动画 =====")
    anim_gif_path = os.path.join(out_dir, "anim_elbow_rotate.gif")
    generate_joint_animation(
        model=model,
        betas=betas,
        joint_name="right_elbow",    # 选择右肘关节做弯曲动画
        axis=[0, 1, 0],              # 绕Y轴旋转实现手臂弯曲
        start_rad=0.0,
        end_rad=0.75,
        frame_num=30,
        out_gif_path=anim_gif_path
    )
    # ===========================================================
